...
def is_overlap(boxa, boxb, thresh=confidence):
    anb = {}
...
for fileName in list_of_files:
    ...
    for (startX, startY, endX, endY) in boxes:
        eastBoxes.append((startX, startY, endX, endY))

    # print(GoogleOCR.detect_handwrite(fileName))

    googleResponse = GoogleOCR.detect_handwrite(fileName)

    for (xMin, yMin, xMax, yMax) in googleResponse:
        googleBoxes.append((xMin, yMin, xMax, yMax))
    ...
    for (startX, startY, endX, endY) in boxes:
        ...
        for (xMin, yMin, xMax, yMax) in googleResponse:
            ...
            if is_overlap(boxa, boxb):
                boxesWithOverlap += 1
                try:
                    googleBoxes.remove((xMin, yMin, xMax, yMax))
                    eastBoxes.remove((startX, startY, endX, endY))
                except ValueError:
                    nothing = 0

    falsePositiveBoxes += len(eastBoxes)
    faseNegativeBoxes += len(googleBoxes)

print("Current tolerance: " + str(confidence))
